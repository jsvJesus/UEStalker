...
void AMasterCharacter::BeginPlay()
{
	...
	{
		InventoryComponent->OnInventoryChanged.AddDynamic(this, &AMasterCharacter::OnInventoryChangedEvent);
	}
...
/*void AMasterCharacter::UpdateAim(float DeltaSeconds)
{
	...
	const FVector NewLoc = FMath::VInterpTo(CurLoc, TargetRel.GetLocation(), DeltaSeconds, CurrentAimSettings.MeshInterpSpeed);
	const FQuat   NewRot = FMath::QInterpTo(CurRot, TargetRel.GetRotation(), DeltaSeconds, CurrentAimSettings.MeshInterpSpeed);
...
void AMasterCharacter::UpdateWeaponVisuals()
{
	...
	if (TObjectPtr<AActor>* Ptr = GetHeldActorPtrBySlot(ActiveWeaponSlot))
	{
		...
		// на всякий — обновим ссылку на ItemObject, чтобы магазин/патроны были актуальны
		if (AMasterWeaponActor* WeaponActor = Cast<AMasterWeaponActor>(*Ptr))
		{
			WeaponActor->SetWeaponItemObject(IsValid(EquipmentComponent) ? EquipmentComponent->GetItemInSlot(ActiveWeaponSlot) : nullptr);
		}
	}
...
AActor* AMasterCharacter::SpawnHeldActorFromItem(UItemObject* Item)
{
	...
	// связать актор оружия с его ItemObject (для магазина/патронов)
	if (AMasterWeaponActor* WeaponActor = Cast<AMasterWeaponActor>(A))
	{
		WeaponActor->SetWeaponItemObject(Item);
	}
...
void AMasterCharacter::OnInventoryChangedEvent()
{
	// если меняли магазин/патроны через Drag&Drop — обновим актор в руках
	if (!IsValid(EquipmentComponent))
	{
		return;
	}
	...
	if (TObjectPtr<AActor>* HeldPtr = GetHeldActorPtrBySlot(ActiveWeaponSlot))
	{
		if (AMasterWeaponActor* WeaponActor = Cast<AMasterWeaponActor>(HeldPtr->Get()))
		{
			WeaponActor->SetWeaponItemObject(ActiveItem);
		}
	}
