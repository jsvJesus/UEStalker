...
UENUM(BlueprintType)
enum class ELeftHandIKMode : uint8
{
	None      UMETA(DisplayName="None"),
	Grip      UMETA(DisplayName="Grip"),
	ReloadMag UMETA(DisplayName="ReloadMag"),
};
...
public:
	...
	// ===== 1P Tick Sync (руки/оружие без рассинхрона) =====
	void ApplyFirstPersonTickSync();
	void ApplyTickSyncToHeldActor(AActor* HeldActor);
	
	// ===== Optional Montage Sync (если есть отдельные анимации оружия) =====
	void StartSyncedMontages(UAnimMontage* ArmsMontage, UAnimMontage* WeaponMontage, float PlayRate);
	void TickSyncedMontages();
	void StopSyncedMontages();
	
	UPROPERTY(Transient)
	TObjectPtr<UAnimInstance> SyncedArmsAnim = nullptr;
	
	UPROPERTY(Transient)
	TObjectPtr<UAnimInstance> SyncedWeaponAnim = nullptr;
	
	UPROPERTY(Transient)
	TObjectPtr<UAnimMontage> SyncedArmsMontage = nullptr;
	
	UPROPERTY(Transient)
	TObjectPtr<UAnimMontage> SyncedWeaponMontage = nullptr;
	
	bool bSyncMontages = false;

public:
	// ===== Left-hand IK (1P) =====
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category="Weapon|IK")
	FName RightHandBoneName = TEXT("r_hand");

	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category="Weapon|IK")
	FName LeftHandBoneName = TEXT("l_hand"); // Кость левой руки для оружия

	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category="Weapon|IK")
	ELeftHandIKMode LeftHandIKMode = ELeftHandIKMode::Grip;

	UFUNCTION(BlueprintPure, Category="Weapon|IK")
	ELeftHandIKMode GetLeftHandIKMode() const { return LeftHandIKMode; }

	UFUNCTION(BlueprintCallable, Category="Weapon|IK")
	void SetLeftHandIKMode(ELeftHandIKMode NewMode);

	UFUNCTION(BlueprintPure, Category="Weapon|IK")
	FName GetRightHandBoneName() const { return RightHandBoneName; }

	UFUNCTION(BlueprintPure, Category="Weapon|IK")
	FName GetLeftHandBoneName() const { return LeftHandBoneName; }

	UFUNCTION(BlueprintPure, Category="Weapon|IK")
	AMasterWeaponActor* GetActiveWeaponActor() const;

	UFUNCTION()
	void OnArmsReloadMontageEnded(UAnimMontage* Montage, bool bInterrupted);
